 ...
 */
public class EncryptedFileChannel extends FileBase {
        ...
        public EncryptedFileChannel(String name, byte[] encryptionKey, FileChannel base, boolean readOnly) {
            ...
            this.name = name;
            this.base = base;
            this.encryptionKey = encryptionKey.clone(); // clone is needed since the given byte array ref is cleared
        ...
        private static void readFully(FileChannel file, long pos, ByteBuffer dst)
            ...
            int len;
            do {
                len = file.read(dst, pos);
                pos += len;
            } while (len != -1 && dst.remaining() > 0);
        }
